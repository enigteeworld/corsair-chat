"use client";

import Link from "next/link";
import { usePathname } from "next/navigation";
import { X, MessageSquare } from "lucide-react";
import { navigation } from "@/lib/navigation";

type MobileDrawerProps = {
  open: boolean;
  onClose: () => void;
};

export default function MobileDrawer({
  open,
  onClose,
}: MobileDrawerProps) {
  const pathname = usePathname();

  return (
    <div
      className={`fixed inset-0 z-[200] lg:hidden ${
        open ? "pointer-events-auto" : "pointer-events-none"
      }`}
    >
      <div
        className={`absolute inset-0 bg-black/72 transition-opacity duration-300 ${
          open ? "opacity-100" : "opacity-0"
        }`}
        onClick={onClose}
      />

      <aside
        className={`absolute left-0 top-0 h-full w-[84vw] max-w-[340px] border-r border-white/10 bg-[#06080d]/95 backdrop-blur-2xl transition-transform duration-300 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between px-5 py-5">
          <Link href="/" className="flex items-center gap-3" onClick={onClose}>
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-cyan-400/20 bg-cyan-500/10 text-cyan-300 shadow-[0_0_30px_rgba(34,211,238,0.12)]">
              <MessageSquare className="h-5 w-5" />
            </div>
            <div>
              <div className="text-lg font-semibold tracking-tight text-white">
                Corsair Chat
              </div>
              <div className="text-sm text-white/55">
                Creative + productivity assistant
              </div>
            </div>
          </Link>

          <button
            onClick={onClose}
            className="rounded-xl border border-white/10 p-2 text-white/70 transition hover:bg-white/5 hover:text-white"
            aria-label="Close menu"
          >
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="px-4 pb-6">
          <div className="space-y-2">
            {navigation.map((item) => {
              const Icon = item.icon;
              const active =
                pathname === item.href ||
                (item.href !== "/" && pathname.startsWith(item.href));

              return (
                <Link
                  key={item.href}
                  href={item.href}
                  onClick={onClose}
                  className={`flex items-center gap-3 rounded-2xl px-4 py-4 transition ${
                    active
                      ? "bg-white/10 text-white"
                      : "text-white/70 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  <Icon className="h-5 w-5" />
                  <span className="text-lg font-medium">{item.label}</span>
                </Link>
              );
            })}
          </div>

          <div className="mt-8 border-t border-white/10 pt-6">
            <div className="mb-3 text-sm font-semibold uppercase tracking-[0.18em] text-white/35">
              About Corsair Chat
            </div>
            <p className="text-sm leading-6 text-white/60">
              A standalone assistant app for writing, research, planning, and
              everyday productivity. No finance or trading access.
            </p>
          </div>
        </div>
      </aside>
    </div>
  );
}